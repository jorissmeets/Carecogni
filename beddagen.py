...
# Bestand om e-mails op te slaan
...
# Functie om e-mails op te slaan
def save_email(email):
    if not os.path.exists(EMAIL_FILE):
        pd.DataFrame(columns=["email"]).to_csv(EMAIL_FILE, index=False)
    df = pd.read_csv(EMAIL_FILE)
    if email not in df["email"].values:
        new_row = pd.DataFrame({"email": [email]})
        df = pd.concat([df, new_row], ignore_index=True)
        df.to_csv(EMAIL_FILE, index=False)
        return True
    return False

# E-mail invoerveld
st.title("Welkom bij de Ligdagen Voorspelling Demo")
st.write("Vul je e-mailadres in om toegang te krijgen tot de demo.")

email = st.text_input("E-mailadres", placeholder="jouw@email.com")

if "access_granted" not in st.session_state:
    st.session_state.access_granted = False

# Controleer en valideer e-mail
if st.button("Verstuur") and not st.session_state.access_granted:
    if is_valid_email(email):
        if save_email(email):
            st.success("Dankjewel! Je krijgt toegang tot de demo.")
            st.session_state.access_granted = True
        else:
            st.warning("Dit e-mailadres is al geregistreerd. Je krijgt toegang tot de demo.")
            st.session_state.access_granted = True
    else:
        st.error("Ongeldig e-mailadres. Probeer het opnieuw.")

# Alleen toegang als toegang is verleend
if st.session_state.access_granted:
    # Voeg hier je originele code toe
    st.subheader("Start van de demo:")
    ...
    leeftijd = leeftijd - 17
    # Input data
    input_data = [leeftijd, bmi, asa, dyspnoe_score, ses_score]

    # Averages (replace with real values if available)
    average_data = [40, 25, 3, 3, 0]

    # Maximum values for each parameter
    max_values = [85, 45, 5, 5, 200]  # SES normalized to run from 0 to 200
